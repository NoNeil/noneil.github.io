...
model.add(Dense(num_classes, activation='softmax'))
```
1. 第1个卷积层：有25个filter，每个filter的参数个数为`3*3=9`，共有`25*9=245`个参数
2. 第2个卷积层：有50个filter，每个filter的参数个数为`3*3*25=225`，共有`225*9=11250`个参数
<img src="cnn_param_2.png" width="500px">

TensorFlow中在每个filter上多加了一个参数，所以每个filter的参数个数都要**加1**。**这个参数是干嘛的呢？**
1. 第1个卷积层：每个filter的参数个数为`3*3+1=10`，共有`25*10=250`个参数
2. 第2个卷积层：每个filter的参数个数为`3*3*25+1=226`，共有`226*9=11300`个参数
